...
def test_cutedsl_codegen_partial_reduce_named_barrier():
    """The partial scalar AllReduce must emit NamedBarrier(participants, id)
    with the barrier ID carried on the barrier type (two template args), and
    the CuTeDSL Python NamedBarrier must accept both."""
    artifact = _lower_cutedsl_partial_reduce()
    assert "tl.NamedBarrier(64, 1)" in artifact.kernel_source
